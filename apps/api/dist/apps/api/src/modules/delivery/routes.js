...
deliveryRouter.post("/riders", requireAuth, allowRoles("MASTER_ADMIN", "ADMIN", "MANAGER"), async (req, res) => {
    ...
    if (pool) {
        const user = req.user;
        const created = await pool.query(`INSERT INTO riders (branch_id, full_name, phone, pay_rate, rate_type, is_active)
       VALUES ($1,$2,$3,$4,$5,TRUE)
       RETURNING id, full_name as "name", phone, pay_rate as "payRate", rate_type as "type", is_active as "isActive"`, [user.branchId, parsed.data.name, parsed.data.phone, parsed.data.payRate || 0, parsed.data.type || "Per Delivery"]);
...
deliveryRouter.put("/riders/:id", requireAuth, allowRoles("MASTER_ADMIN", "ADMIN", "MANAGER"), async (req, res) => {
    ...
    if (pool) {
        await pool.query(`UPDATE riders SET full_name=COALESCE($1, full_name), phone=COALESCE($2, phone), 
        ...
        WHERE id=$7`, [name, phone, payRate, type, deliveriesDone, isActive, id]);
    ...
    if (payRate !== undefined)
        rider.payRate = payRate;
    if (type !== undefined)
        rider.type = type;
    if (deliveriesDone !== undefined)
        rider.deliveriesDone = deliveriesDone;
