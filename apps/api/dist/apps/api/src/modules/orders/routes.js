    ...
    if (pool) {
        ...
        try {
            ...
            if (!finalCustomerId && parsed.data.customerPhone && parsed.data.customerName) {
                const custRes = await client.query(`INSERT INTO customers (branch_id, full_name, phone) VALUES ($1, $2, $3) RETURNING id`, [user.branchId, parsed.data.customerName, parsed.data.customerPhone]);
                finalCustomerId = custRes.rows[0].id;
                if (parsed.data.customerAddress) {
                    await client.query(`INSERT INTO customer_addresses (customer_id, full_address, is_default) VALUES ($1, $2, true)`, [finalCustomerId, parsed.data.customerAddress]);
                }
            }
            try {
                await client.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS tax_rate NUMERIC(6,4) DEFAULT 0");
            }
            catch (e) { }
            ...
            const created = await client.query(`INSERT INTO orders (branch_id, order_no, customer_id, channel, status, placed_by_staff_id, notes, customer_name, customer_phone, customer_address, tax_rate)
         VALUES ($1,$2,$3,$4,'PENDING',$5,$6,$7,$8,$9,$10)
         RETURNING id, order_no, branch_id, customer_id, channel, status, placed_by_staff_id, placed_at, notes, tax_rate`, [user.branchId, orderNo, finalCustomerId ?? null, parsed.data.channel, user.sub, parsed.data.notes ?? null, finalCustomerName ?? null, parsed.data.customerPhone ?? null, parsed.data.customerAddress ?? null, parsed.data.taxRate || 0]);
            for (const item of parsed.data.items) {
                const unitPrice = item.unitPrice || 0;
                await client.query(`INSERT INTO order_items (order_id, menu_item_id, item_name_snapshot, unit_price_snapshot, quantity, line_total)
           VALUES ($1,$2,$3,$4,$5,$6)`, [created.rows[0].id, item.menuItemId, item.itemName || `ITEM-${item.menuItemId}`, unitPrice, item.quantity, unitPrice * item.quantity]);
            }
            const items = await client.query(`SELECT menu_item_id as "menuItemId", item_name_snapshot as "itemName", unit_price_snapshot as "unitPrice", quantity, NULL::text as note
         FROM order_items WHERE order_id=$1`, [created.rows[0].id]);
...
ordersRouter.get("/", requireAuth, async (_req, res) => {
    if (pool) {
        ...
        const result = await pool.query(`SELECT o.id, o.order_no as "orderNo", o.channel, o.status, o.placed_at as "placedAt", o.tax_rate as "taxRate", o.payment_method as "paymentMethod",
              COALESCE(
                json_agg(
...
ordersRouter.post("/:id/settle", requireAuth, allowRoles("MASTER_ADMIN", "ADMIN", "MANAGER", "CASHIER"), async (req, res) => {
    ...
    if (parsed.data.taxRate !== undefined)
        order.taxRate = parsed.data.taxRate;
    if (parsed.data.customerId !== undefined)
        order.customerId = parsed.data.customerId;
    if (parsed.data.customerName !== undefined)
        order.customerName = parsed.data.customerName;
    if (parsed.data.customerPhone !== undefined)
        order.customerPhone = parsed.data.customerPhone;
    if (parsed.data.customerAddress !== undefined)
        order.customerAddress = parsed.data.customerAddress;
    order.status = "COMPLETED";
    order.completedAt = new Date().toISOString();
    if (parsed.data.paymentMethod === "KHATA" && parsed.data.customerId) {
        const customer = store.customers.get(parsed.data.customerId);
        if (customer) {
            const totalAmount = order.items.reduce((sum, i) => sum + ((i.unitPrice || 0) * i.quantity), 0);
            customer.khataBalance += totalAmount;
        }
    }
    saveStore();
...
ordersRouter.put("/:id", requireAuth, allowRoles("MASTER_ADMIN", "ADMIN", "MANAGER", "CASHIER", "WAITER"), async (req, res) => {
    const id = Number(req.params.id);
    const bodySchema = z.object({
    ...
    if (parsed.data.channel)
        order.channel = parsed.data.channel;
    if (parsed.data.taxRate !== undefined)
        order.taxRate = parsed.data.taxRate;
...
ordersRouter.put("/:id/status", requireAuth, async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ message: parsed.error.flatten() });
    if (pool) {
        const field = parsed.data.status === "PREPARING"
            ? "prep_started_at"
            : parsed.data.status === "READY"
                ...
                : parsed.data.status === "OUT_FOR_DELIVERY"
                    ...
                    : parsed.data.status === "COMPLETED"
                        ? "completed_at"
                        : null;
        const result = await pool.query(`UPDATE orders SET status=$1,
       ...
       RETURNING id, order_no as "orderNo", status, placed_at as "placedAt",
                 ...
                 out_for_delivery_at as "outForDeliveryAt", completed_at as "completedAt"`, [parsed.data.status, field, Number(req.params.id), parsed.data.reason || null]);
