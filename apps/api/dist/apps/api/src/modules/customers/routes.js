...
customersRouter.post("/", requireAuth, async (req, res) => {
    ...
    const user = req.user;
    if (pool) {
        const result = await pool.query(`INSERT INTO customers (branch_id, full_name, phone, address, loyalty_points, khata_balance)
       VALUES ($1,$2,$3,$4,0,0)
       RETURNING id, full_name as "fullName", phone, address, loyalty_points as "loyaltyPoints", khata_balance as "khataBalance"`, [user.branchId, parsed.data.fullName, parsed.data.phone, parsed.data.address || null]);
...
customersRouter.get("/", requireAuth, async (req, res) => {
    ...
    if (pool) {
        const user = req.user;
        const result = await pool.query(`SELECT id, full_name as "fullName", phone, address, loyalty_points as "loyaltyPoints", khata_balance as "khataBalance"
       FROM customers
       WHERE branch_id=$1 
         AND ($2::text IS NULL OR phone ILIKE '%' || $2 || '%' OR full_name ILIKE '%' || $2 || '%')
       ORDER BY id DESC`, [user.branchId, query ?? null]);
