...
analyticsRouter.get("/overview", requireAuth, (req, res) => {
    ...
    const filtered = allOrders.filter((o) => {
        ...
        return placed >= startDate && placed <= endDate;
    });
    ...
    const dailySales = Array.from(filtered.reduce((acc, o) => {
        const date = new Date(o.placedAt).toISOString().split('T')[0];
        if (!acc.has(date))
            acc.set(date, { date, successfulOrders: 0, cancelledOrders: 0, revenue: 0, taxCollected: 0 });
        ...
        if (statusStr === "CANCELLED" || statusStr === "FAILED_DELIVERY") {
            ...
            const subtotal = o.items.reduce((sum, i) => sum + (i.quantity * (i.unitPrice || 0)), 0);
            stat.revenue += subtotal;
            stat.taxCollected += subtotal * (o.taxRate || 0);
        }
        return acc;
    }, new Map()).values()).sort((a, b) => a.date.localeCompare(b.date));
    const totalRevenue = successfulOrders.reduce((sum, o) => sum + o.items.reduce((s, i) => s + (i.quantity * (i.unitPrice || 0)), 0), 0);
    const totalTax = successfulOrders.reduce((sum, o) => {
        const subtotal = o.items.reduce((s, i) => s + (i.quantity * (i.unitPrice || 0)), 0);
