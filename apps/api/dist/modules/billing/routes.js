...
billingRouter.post("/from-order/:orderId", requireAuth, allowRoles("MASTER_ADMIN", "ADMIN", "CASHIER"), async (req, res) => {
    ...
    if (pool) {
        ...
        const bill = await pool.query(`INSERT INTO bills (order_id, subtotal, discount_total, tax_total, tip_total, delivery_charges, grand_total)
       VALUES ($1,$2,0,$3,0,$4,$5)
       RETURNING id, order_id as "orderId", subtotal, discount_total as "discountTotal", tax_total as "taxTotal",
                 tip_total as "tipTotal", delivery_charges as "deliveryCharges", grand_total as "grandTotal"`, [orderId, subtotal, taxTotal, deliveryCharges, grandTotal]);
        return res.status(201).json({ ...bill.rows[0], payments: [] });
    }
    const order = store.orders.get(orderId);
    if (!order)
        return res.status(404).json({ message: "Order not found" });
    if (Array.from(store.bills.values()).some((b) => b.orderId === order.id))
        return res.status(400).json({ message: "Bill already exists" });
    store.seq.bill += 1;
    const subtotal = order.items.reduce((sum, item) => sum + item.quantity * 10, 0);
    const taxTotal = subtotal * 0.16;
    const deliveryCharges = order.channel === "DELIVERY" ? 5 : 0;
    const bill = { id: store.seq.bill, orderId: order.id, subtotal, discountTotal: 0, taxTotal, tipTotal: 0, deliveryCharges, grandTotal: subtotal + taxTotal + deliveryCharges, payments: [] };
...
billingRouter.post("/:id/payments", requireAuth, allowRoles("MASTER_ADMIN", "ADMIN", "CASHIER"), async (req, res) => {
    ...
    if (pool) {
        ...
        const rows = await pool.query(`SELECT b.id, b.order_id as "orderId", b.subtotal, b.discount_total as "discountTotal", b.tax_total as "taxTotal",
    ...
    if (parsed.data.method === "KHATA" && parsed.data.customerId) {
        ...
        if (customer)
            customer.khataBalance += parsed.data.amount;
    }
