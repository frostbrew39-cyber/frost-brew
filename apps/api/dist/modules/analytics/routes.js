...
analyticsRouter.get("/overview", requireAuth, (req, res) => {
    ...
    const dailySales = Array.from(filtered.reduce((acc, o) => {
        ...
        if (!acc.has(date))
            acc.set(date, { date, successfulOrders: 0, cancelledOrders: 0, revenue: 0 });
        ...
        if (statusStr === "CANCELLED" || statusStr === "FAILED") {
            ...
            stat.revenue += o.items.reduce((sum, i) => sum + (i.quantity * (i.unitPrice || 0)), 0);
        }
        return acc;
    ...
    res.json({
        ...
        grossSales: successfulOrders.reduce((sum, o) => sum + o.items.reduce((s, i) => s + (i.quantity * (i.unitPrice || 0)), 0), 0),
