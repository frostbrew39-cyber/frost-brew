...
const rawSocket = import.meta.env.VITE_SOCKET_URL?.trim().replace(/\/+$/, "");
function inferSocketFromApiUrl(apiUrl) {
 ...
 */
export const API_V1_URL = rawApi && rawApi.length > 0
    ? rawApi
    : import.meta.env.DEV
        ? `${devApiHost()}/api/v1`
        : "";
 ...
 */
export const SOCKET_URL = rawSocket && rawSocket.length > 0
    ? rawSocket
...
if (import.meta.env.PROD && !rawApi) {
    ...
    console.error("[POS] Production build is missing VITE_API_URL. Set it in Vercel to your Render API URL (e.g. https://your-api.onrender.com/api/v1).");
}
...
export function createPosSocket() {
